...
const myBot = ['u4d4b6b57a29acd07e27b5b0f31a2e64e'];
...
\n";
		...
		}else if(txt == "leave" && !isAdminOrBot(seq.from_)){this._sendMessage(seq,"Not permitted !");}
		...
		if(txt == "kick aku" && seq.toType == 2 && !isBanned(banList, seq.from_) && this.stateStatus.kick == 1){
        ...
        if(txt === 'Ruchi' && this.stateStatus.kick == 1 && isAdminOrBot(seq.from_) && seq.toType == 2) {
            let { listMember } = await this.searchGroup(seq.to);
            for (var i = 0; i < listMember.length; i++) {
        ...
        }else if(txt === 'Ruchi' && !isAdminOrBot(seq.from_) && seq.toType == 2){this._sendMessage(seq,"Not permitted !");}
...
≽ VERSION J.S\n\
		...
		if(txt == "baba" && isAdminOrBot(seq.from_)){
			for(var i = 0; i < banList.length; i++){
		...
		}else if(txt == "baba" && !isAdminOrBot(seq.from_)){this._sendMessage(seq,"Not permitted !");}
